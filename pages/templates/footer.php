<div class="socialwrapper">
    <!--<h4>Follow me on</h4>-->
    <ul class="social-icons">
        <li>
            <a href="http://www.facebook.com/toby.ziegler.3" class="social-icon"> <i class="fa fa-lg fa-facebook"></i></a>
        </li>
        <li>
            <a href="http://twitter.com/_TobyZiegler" class="social-icon"> <i class="fa fa-lg fa-twitter"></i></a>
        </li>
        <li>
            <a href="https://www.linkedin.com/in/toby-ziegler-24190994?trk=profile-badge" class="social-icon"> <i class="fa fa-lg fa-linkedin"></i></a>
        </li>
        <li>
            <a href="//plus.google.com/u/0/110499189687437609705?prsrc=3" class="social-icon"> <i class="fa fa-lg fa-google-plus"></i></a>
        </li>
        <!--
        <li>
            <a href="" class="social-icon"> <i class="fa fa-lg fa-rss"></i></a>
        </li>
        <li>
            <a href="" class="social-icon"> <i class="fa fa-lg fa-youtube"></i></a>
        </li>
-->
    </ul>
</div>

<p class="copyright">&copy; 2018 <a href=http://tobyziegler.com>Toby Ziegler</a>, all rights reserved.</p>
<!--linkedin code: -->
